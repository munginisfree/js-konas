describe('scope 대해서 학습합니다.', function () {
  ...
  it('default parameter에 대해 확인합니다.', function () {
    ...
    //above code same as below code
    function defaultParameters(num){
      if(num===undefined){
        num = 5;
      }
      return num;
  ...
  it('lexical scope와 closure에 대해 다시 확인합니다.', function () {
    ...
    function outerFn() {
      let age = 24; //26 왜냐하면 함수 outerFn안에서 innerFn이 실행되기 때문에 24가 26으로 바뀜
      name = 'jimin';
      let height = 178;
    ...
    const innerFn = outerFn(); //함수자체를 할당하고 있음
    //const innerFn = innerFn; 이 말과 동일함
